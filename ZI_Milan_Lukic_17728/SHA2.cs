...
namespace ZI_Milan_Lukic_17728
{
    internal class SHA2
    {
        ...
        private void Transform(ref SHA256_CTX ctx, byte[] data)
        {
        ...
        private void Init(ref SHA256_CTX ctx)
        {
            ctx.datalen = 0;
            ctx.bitlen[0] = 0;
            ctx.bitlen[1] = 0;
        ...
        private void Update(ref SHA256_CTX ctx, byte[] data, uint len)
        {
        ...
        private void Final(ref SHA256_CTX ctx, byte[] hash)
        {
            uint i = ctx.datalen;
        ...
        public string Hash(string data)
        {
            ...
            Init(ref ctx);
            Update(ref ctx, Encoding.Default.GetBytes(data), (uint)data.Length);
            Final(ref ctx, hash);
